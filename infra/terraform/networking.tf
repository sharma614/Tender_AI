...
# ---------------------------------------------------------------------------
# Private Subnets (ECS tasks, RDS, Lambda run here)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# NAT Gateway (private subnets → internet for pip install, Gemini API)
# ---------------------------------------------------------------------------
...
# ALB — accepts HTTPS (443) and HTTP (80) from internet
...
# ECS API — only accepts traffic from the ALB
resource "aws_security_group" "api" {
  name        = "${var.project_name}-api-sg"
  description = "Allow traffic from ALB to FastAPI ECS task"
  vpc_id      = aws_vpc.main.id
...
# RDS — only accepts PostgreSQL from API + Worker
...
# Lambda — same private subnet access as ECS; outbound to SQS (via VPC endpoint or NAT)
resource "aws_security_group" "lambda" {
  name        = "${var.project_name}-lambda-sg"
  description = "Lambda trigger security group"
  vpc_id      = aws_vpc.main.id
